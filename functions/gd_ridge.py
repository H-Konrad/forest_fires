...
def energy_function2(inputs, outputs, weights, alpha):
    mse = (np.linalg.norm(inputs@weights - outputs)**2)/(2 * len(inputs))
    gradient = (inputs.T@(inputs@weights-outputs)) + alpha * weights
    return gradient, mse

def gradient_descent2(training_data, testing_data, weights, alpha, step_size = 0.5, iterations = 1000):
    for i in range(iterations + 1):
        gradient_list, mse_train_list, mse_test_list = [], [], []
        for j in range(len(training_data)):
            gradient, mse = energy_function2(training_data[j][0], training_data[j][1], weights, alpha)
